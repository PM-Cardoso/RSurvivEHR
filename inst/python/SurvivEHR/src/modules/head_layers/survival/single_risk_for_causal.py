...
class CausalODESurvSingleRiskLayer(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim, num_risks, n=15, concurrent_strategy=None, device="cpu"):
        ...
        self.sr_ode = nn.ModuleList(
            [ODESurvSingle(cov_dim=hidden_dim,
                           hidden_dim=None,
                           device=device,
                           n=n) 
             for _ in range(num_risks)]
        )             # do not include pad token as an event 
    ...
    def _predict_cdf(self,
        ...
        # NOTE: hidden_states has already been passed through enc_layer by the
        # caller (predict()), so we do NOT apply enc_layer here again.
