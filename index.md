...
`survivehrR` is a standalone R package that lets you run the
**SurvivEHR** transformer survival model directly from R data frames. It
requires no FastEHR installation; all Python model code is vendored
inside the package.
...
6.  [Preparing fine-tune context (avoiding
    leakage)](#preparing-fine-tune-context)
