...
cfg <- survivehr_config(
  ...
  time_scale    = 1.0   # backbone normalisation: age / 1.0 — keeps year values as-is
  # outcome_horizon is set per fine-tune call (e.g. 5.0 for 5-year risk)
)
```
...
| `time_scale` | **Backbone age normalisation only.** Every raw event age is divided by this before entering the model (`age_norm = raw_age / time_scale`). Use `1.0` (default) when ages are in years. Stored in the pretrained bundle and **inherited automatically** at fine-tune time — do not set it in a fine-tune config. |
| `outcome_horizon` | **Fine-tune only.** Length of the ODE prediction window in the same units as `age`. The survival ODE integrates from 0 to `outcome_horizon`, so `cdf_last` = risk at that horizon. Can differ freely from `time_scale` — e.g. `time_scale = 1.0` (year normalisation) with `outcome_horizon = 5.0` (5-year risk). Defaults to `time_scale` if not set. |
...
cfg_cr <- survivehr_config(
  ...
  outcome_horizon = 5.0  # 5-year prediction window (time_scale=1.0 inherited from pt)
)
...
| Column | Description |
|----|----|
| `{outcome}_cdf_last` | Cumulative incidence at the **end** of the prediction window. `cdf_last` equals the risk at `outcome_horizon` raw time units from the patient’s last recorded event. |
...
> steps from 0 to 1 in normalised time (0 to `outcome_horizon` in raw
> units). `eval_times` values are snapped to the nearest grid point,
> giving a resolution of `outcome_horizon / 999` (≈ 0.005 years with
> `outcome_horizon = 5.0`).
