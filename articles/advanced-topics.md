...
`<PAD>` is always index 0 because SurvivEHR’s
`SplitDynamicEmbeddingLayer` uses `padding_idx = 0` internally.

At prediction time any event code not present in `event_vocab` is
replaced by `<UNK>` (index 1), provided `include_unk = TRUE`.

> **Pre-train only.** `include_unk` and `include_cls_sep` are fixed when
> the vocabulary is built during
> [`survivehr_pretrain()`](https://pm-cardoso.github.io/RSurvivEHR/reference/survivehr_pretrain.md).
> The values stored in the bundle are reused automatically at fine-tune
> and prediction time; any `include_unk` / `include_cls_sep` values in a
...
In practice, leave `surv_layer = "competing-risk"` (the default) unless
you have a specific reason to change it, and use `risk_model` in
[`survivehr_finetune()`](https://pm-cardoso.github.io/RSurvivEHR/reference/survivehr_finetune.md)
to choose `"competing-risk"` (multiple outcomes) or `"single-risk"` (one
outcome).
...
**During pre-training**, this is the only role of `time_scale` — it
controls how ages are scaled before being fed to the transformer. There
is no prediction window during pre-training.
...
so the ODE integrates over $`[0, 1]`$ which maps to
$`[0, \texttt{time\_scale}]`$ raw years (or whatever unit your ages are
in).

Use `time_scale = 1.0` (default) when ages are in **years**.  
Use `time_scale = 365.25` when ages are in **days** (e.g. 18 262 days ≈
50 years).

The `time_scale` is stored in the model bundle and automatically applied
at fine-tune and predict time, so you only need to set it once in
[`survivehr_config()`](https://pm-cardoso.github.io/RSurvivEHR/reference/survivehr_config.md).
...
predict time from the saved bundle.

------------------------------------------------------------------------

## Using a fixed vocabulary

If you have pre-computed a vocabulary (e.g., from a previous run or from
a shared codelist), pass it as `event_vocab`:
...
from the union of all site codelists, then fix it so that model weights
are comparable.
...
The `DAYS_SINCE_BIRTH` alias exists for compatibility with datasets
where ages are stored as days rather than years. If you use it, set
`time_scale = 365.25` in
[`survivehr_config()`](https://pm-cardoso.github.io/RSurvivEHR/reference/survivehr_config.md)
so ages are normalised to year-scale before entering the model.

------------------------------------------------------------------------

## Full worked example

This section reproduces the complete pipeline from the Getting Started
vignette in a single self-contained block, demonstrating every best
practice: leakage-free context construction, targets defined before
context filtering, and the competing-risk fine-tune on two outcomes.
...
**Important — value scale**: `RSurvivEHR` now applies automatic internal
standardisation for numeric values **per event type**. During
pre-training, non-`NA` `value` rows are used to fit event-specific
mean/sd statistics. These statistics are stored in the model bundle and
reused in fine-tuning and prediction so all downstream value inputs are
transformed consistently.

Returned value outputs (`generated_value`, `predicted_value_mean`,
`predicted_value_sd`) are automatically back-transformed to original
units, so no manual pre-normalisation or post-hoc back-transform is
required.
...
See
[`vignette("python_dependencies", package = "reticulate")`](https://rstudio.github.io/reticulate/articles/python_dependencies.html)
for full details.
