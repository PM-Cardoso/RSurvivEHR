...
true event ranked among highest-risk (good prediction) - IEC ≈ 0.5: true
event ranked in middle of risk distribution - IEC ≈ 0.0: true event
ranked among lowest-risk (poor prediction)
...
**New files:** `inst/python/iec_metrics.py`,
`inst/python/test_iec_metrics.py`, `R/metrics_iec.R`,
`R/predict_risks.R`, `R/evaluate_iec.R`,
`tests/testthat/test-metrics_iec.R`

**Changed files:** `inst/python/survivehr_backend.py` (added
`extract_pretrain_risk_scores()`), `NAMESPACE` (new exports and S3
methods)
