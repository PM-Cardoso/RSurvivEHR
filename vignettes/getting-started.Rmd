...
| `time_scale` | **Sets both the age normalisation divisor and the prediction window length.** Ages are divided by this value before entering the model; the survival ODE evaluates over a normalised [0, 1] grid mapping back to [0, `time_scale`] in your age units. Use `5.0` for a 5-year window with ages in years, `1.0` for 1-year. Stored in every model bundle — `survivehr_predict()` reads it automatically. |
...
> from 0 to 1 in normalised time (0 to `time_scale` in raw units).
> `eval_times` values are snapped to the nearest grid point, giving a
> resolution of `time_scale / 999` (≈ 0.005 years with `time_scale = 5.0`).
