...
> `target_age`, `target_value` respectively).
...
> **Prediction window**: internally, the survival ODE evaluates the CDF on a
> fixed grid of 1 000 equally-spaced time steps from 0 to 1 in *normalised*
> time.  Normalised time 1.0 corresponds to `time_scale` in raw units.  The
> grid is set at model architecture level (not a user parameter).
